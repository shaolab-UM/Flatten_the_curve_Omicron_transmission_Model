...
int main(int argc, char *argv[])
{
  ...
  double RATE_contra[N_AGE], RATE_sus[SIM_MAX][N_AGE], CM_a[N_AGE][N_AGE], CM4Beta[N_AGE][N_AGE], CM[N_AGE][N_AGE], Gamma[7];
  ...
  // VE INPUT
  VE VE_4_S_a, VE_4_V1_a, VE_4_V1E_a, VE_4_V1W_a, VE_4_V2_a, VE_4_V2E_a, VE_4_V2W_a;
  VE VE_4_V3_a, VE_4_V3E_a, VE_4_V3W_a, VE_4_R_a, VE_4_RDelta_a;
  ...
  FUNC_read_VE(12, 0, TMP_VE, &VE_4_S_a);
  FUNC_read_VE(12, 1, TMP_VE, &VE_4_V1_a);
  FUNC_read_VE(12, 2, TMP_VE, &VE_4_V1E_a);
  FUNC_read_VE(12, 3, TMP_VE, &VE_4_V1W_a);
  FUNC_read_VE(12, 4, TMP_VE, &VE_4_V2_a);
  FUNC_read_VE(12, 5, TMP_VE, &VE_4_V2E_a);
  FUNC_read_VE(12, 6, TMP_VE, &VE_4_V2W_a);
  FUNC_read_VE(12, 7, TMP_VE, &VE_4_V3_a);
  FUNC_read_VE(12, 8, TMP_VE, &VE_4_V3E_a);
  FUNC_read_VE(12, 9, TMP_VE, &VE_4_V3W_a);
  FUNC_read_VE(12, 10, TMP_VE, &VE_4_R_a);
  FUNC_read_VE(12, 11, TMP_VE, &VE_4_RDelta_a);

  // NPIILs INPUT
  double NPIIL[7][4];
  char path_NPIILs[500];
  sprintf(path_NPIILs, "%s/NPIILs", CITY_SELECTED);
  FUNC_read_matrix_double(path_NPIILs, 7, 4, NPIIL);
  ...
  for (int sim = 0; sim < SIM_MAX; sim++)
  {
    ...
    FUNC_read_matrix_double(path_CM, N_AGE, N_AGE, CM_a);
    ...
    for (int i = 0; i < N_AGE; i++)
    {
      ...
      {
        CM_a[i][j] = CM_a[i][j] * RATE_sus[sim][i];
        CM[i][j] = CM_a[i][j];
    ...
    for (int t = 0; t < (T_EPISTART + T_EPIDURATION) * T_STEP; t++)
    {
      ...
      if (t > (T_EPISTART - 1) * T_STEP)
      {
          prop = total_infection / POP;
          FUNC_VE_strength(&VE_4_S_a, &VE_4_S, prop);
          FUNC_VE_strength(&VE_4_V1_a, &VE_4_V1, prop);
          FUNC_VE_strength(&VE_4_V1E_a, &VE_4_V1E, prop);
          FUNC_VE_strength(&VE_4_V1W_a, &VE_4_V1W, prop);
          FUNC_VE_strength(&VE_4_V2_a, &VE_4_V2, prop);
          FUNC_VE_strength(&VE_4_V2E_a, &VE_4_V2E, prop);
          FUNC_VE_strength(&VE_4_V2W_a, &VE_4_V2W, prop);
          FUNC_VE_strength(&VE_4_V3_a, &VE_4_V3, prop);
          FUNC_VE_strength(&VE_4_V3E_a, &VE_4_V3E, prop);
          FUNC_VE_strength(&VE_4_V3W_a, &VE_4_V3W, prop);
          FUNC_VE_strength(&VE_4_R_a, &VE_4_R, prop);
          FUNC_VE_strength(&VE_4_RDelta, &VE_4_RDelta, prop);
      }
      
      // NPIILs inside<60, between, inside>60
      if (t == (T_EPISTART + NPIIL[0][0]) * T_STEP)
      {
        CONTACT_AGE_NPI(CM_a, CM, NPIIL[0][1], NPIIL[0][2], NPIIL[0][3]);
      }
      if (t == (T_EPISTART + NPIIL[1][0]) * T_STEP)
      {
        CONTACT_AGE_NPI(CM_a, CM, NPIIL[1][1], NPIIL[1][2], NPIIL[1][3]);
      }
      if (t == (T_EPISTART + NPIIL[2][0]) * T_STEP)
      {
        CONTACT_AGE_NPI(CM_a, CM, NPIIL[2][1], NPIIL[2][2], NPIIL[2][3]);
      }
      if (t == (T_EPISTART + NPIIL[3][0]) * T_STEP)
      {
        CONTACT_AGE_NPI(CM_a, CM, NPIIL[3][1], NPIIL[3][2], NPIIL[3][3]);
      }
      if (t == (T_EPISTART + NPIIL[4][0]) * T_STEP)
      {
        CONTACT_AGE_NPI(CM_a, CM, NPIIL[4][1], NPIIL[4][2], NPIIL[4][3]);
      }
      if (t == (T_EPISTART + NPIIL[5][0]) * T_STEP)
      {
        CONTACT_AGE_NPI(CM_a, CM, NPIIL[5][1], NPIIL[5][2], NPIIL[5][3]);
      }
      if (t == (T_EPISTART + NPIIL[6][0]) * T_STEP)
      {
        CONTACT_AGE_NPI(CM_a, CM, NPIIL[6][1], NPIIL[6][2], NPIIL[6][3]);
      }
      ...
      // ACCMULATED INFECTION NUMBER
      for (int i = 0; i < N_AGE; i++)
      {
        infe = 0;
        infe = infe+save_NEW_S.I[i]+save_NEW_V1.I[i]+save_NEW_V2.I[i]+save_NEW_V3.I[i];
        infe = infe+save_NEW_V1E.I[i]+save_NEW_V2E.I[i]+save_NEW_V3E.I[i];
        infe = infe+save_NEW_V1W.I[i]+save_NEW_V2W.I[i]+save_NEW_V3W.I[i];
        infe = infe+save_NEW_R.I[i]+save_NEW_RDelta.I[i];
        Infection[i] = Infection[i] + infe;
      }

      total_infection = 0.0;
      for (int i = 0; i< N_AGE; i++)
      {
        total_infection = total_infection + Infection[i];
      }
