...
void FUNC_VE_strength(VE *input_VE, VE *output_VE, double prop)
{
  (*output_VE).Inf = (*input_VE).Inf / pow((prop+1), 2);
  (*output_VE).OT = (*input_VE).OT;
  (*output_VE).Symp = (*input_VE).Symp;
  (*output_VE).Hosp = (*input_VE).Hosp;
  (*output_VE).Death = (*input_VE).Death;
  (*output_VE).rate4V = (*input_VE).rate4V;
  (*output_VE).rate4R2S = (*input_VE).rate4R2S;
}


void CONTACT_AGE_NPI(double origin_CM[][N_AGE], double current_CM[][N_AGE], double index_1, double index_2, double index_3)
{
  for (int i = 0; i < N_AGE; i++)
  {
    for (int j = 0; j < N_AGE; j++)
    {
      if (i < 11 && j < 11)
      {
        current_CM[i][j] = origin_CM[i][j] * index_1;
      }
      if (i > 10 && j < 11)
      {
        current_CM[i][j] = origin_CM[i][j] * index_2;
      }
      if (i < 11 && j > 10)
      {
        current_CM[i][j] = origin_CM[i][j] * index_2;
      ...
      if (i > 10 && j > 10)
      {
        current_CM[i][j] = origin_CM[i][j] * index_3;
      }
    }
  }
}
